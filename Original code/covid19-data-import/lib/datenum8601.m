...
assert(ischar(str)&&isr(size(str)),'First input <str> must be a 1xN char.')
%
if nargin<2 || (ischar(tkn)&&isscalar(tkn)&&ismember(tkn,'T @_'))
	...
	wco = '^(\*?)(Y(W(D~~)?)?|y(n~~|m(d~~)?)?)(\d*)$';
	rgx = strrep(wco,'~~',hms);
	assert(ischar(tkn)&&isr(size(tkn)),'Second input must be a character row vector.')
	tmp = regexp(tkn,rgx,'once','tokens');
	assert(numel(tmp)>0,'Second input is not a supported token (read the help): ''%s''',tkn)
    assert(numel(tmp)==3,ero)
	rgx = dn8601User(tmp{:});
	isY = strcmp(tmp{2},'Y');
end
%
...
%
assert(all(cellfun('length',tkx)==4),ero)
%
tkx = vertcat(tkx{:});
tky = tkx(:,1); % year
tkf = tkx(:,3); % fraction
